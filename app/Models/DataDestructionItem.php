...
class DataDestructionItem extends Model
{
    use HasFactory;

    protected $table = 'data_destruction_items';
    ...
    public function getParsedItemsAttribute(): array
    {
        ...
        for ($i = 0; $i < $maxCount; $i++) {
            $nameVal = $names[$i] ?? ($names[0] ?? $this->name ?? 'Media Item');
    ...
    {
        $names = array_values(array_filter(array_map('trim', preg_split('/[\n\r,]+/', $this->name ?? '')), fn($val) => $val !== ''));
        return $names[0] ?? ($this->name ?: 'Media Item');
    }
