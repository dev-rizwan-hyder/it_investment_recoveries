...
<div class="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
    ...
    <div class="mb-6 flex items-center gap-3">
        ...
        <div>
            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">IT Assets</span>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="dashboard-card rounded-3xl border border-slate-200/80 bg-white p-6 sm:p-8">
                <div class="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 pb-5 mb-5">
                    <div>
                        <h2 class="text-xl font-black text-slate-950">{{ $item->name ?: 'IT Asset' }}</h2>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-6">
                    <div>
                        ...
                        <p class="mt-1 text-slate-900 font-extrabold">{{ $item->category ?: 'IT Assets' }}</p>
                    </div>
                    ...
                    @endif
                    <div>
                        <p class="text-xs font-bold text-slate-400 uppercase">Condition</p>
                        <p class="mt-1 text-slate-900 font-extrabold">{{ $item->condition ?: 'N/A' }}</p>
                    </div>
                    <div>
                ...
                <div class="mt-8 border-t border-slate-100 pt-6">
                    ...
                    @else
                        <p class="text-xs text-slate-400 font-medium">No individual serial numbers logged for this IT asset.</p>
                    @endif
                </div>
            ...
            <div class="dashboard-card rounded-3xl border border-slate-200/80 bg-white p-6">
                ...
                @if (!empty($photos) && is_array($photos))
                    <div class="grid grid-cols-2 gap-3">
                        @foreach ($photos as $photo)
                            ...
                            @if ($isImage)
                                <a href="{{ $photoSrc }}" target="_blank" class="group relative block aspect-square overflow-hidden rounded-2xl border border-slate-100 bg-slate-50 hover:border-indigo-200 transition">
                                    <img src="{{ $photoSrc }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-105" alt="Asset photo">
                            ...
                            @else
                                ...
                                @endphp
                                <a href="{{ $photoSrc }}" target="_blank" download="{{ $filename }}" class="group relative flex flex-col items-center justify-center aspect-square rounded-2xl border border-slate-200 bg-slate-50 p-3 text-center hover:border-indigo-300 hover:bg-slate-100/80 transition">
                                    ...
                                    <span class="mt-1 text-[9px] font-extrabold uppercase text-slate-400 group-hover:text-indigo-600">Download</span>
                                </a>
                            @endif
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-10 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
                        ...
                        <p class="text-slate-400 text-xs font-semibold">No photos or files logged for this IT asset</p>
                    </div>
                @endif
            </div>
        </div>
