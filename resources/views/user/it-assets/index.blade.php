...
<div class="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8 space-y-8">
        ...
        <div class="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between z-10">
            <div class="max-w-2xl space-y-3">
                ...
                <p class="text-sm leading-relaxed text-slate-300 sm:text-base">
                    Monitor inventory, technical specifications, serial numbers, and refurbishment status of your processed IT equipment.
                </p>
            </div>
        </div>
    </section>
    ...
    @if ($items->count() > 0)
        @php
            $grandQuantity = $items->sum('quantity');
            $grandWeight = $items->sum('weight');
        @endphp
        <section class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div class="portal-card rounded-2xl p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-extrabold uppercase tracking-wider text-slate-400">Total Asset Entries</p>
            ...
            <div class="portal-card rounded-2xl p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-extrabold uppercase tracking-wider text-slate-400">Processed Quantity</p>
                        <p class="mt-1.5 text-2xl font-black text-indigo-600">{{ number_format($grandQuantity) }}</p>
                    </div>
            ...
            <div class="portal-card rounded-2xl p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-extrabold uppercase tracking-wider text-slate-400">Total Weight</p>
                        <p class="mt-1.5 text-2xl font-black text-slate-900">{{ number_format($grandWeight, 1) }} <span class="text-xs text-slate-400">lbs</span></p>
                    </div>
        ...
        <div class="p-5 border-b border-slate-100 bg-slate-50/60 flex flex-col sm:flex-row gap-4 justify-between items-center">
            <div class="relative w-full sm:w-80">
                ...
                <input type="text" 
                       ...
                       class="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-900 placeholder-slate-400 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition">
            </div>

            <div class="flex items-center gap-3 w-full sm:w-auto">
                <select id="assetStatusFilter" class="px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition w-full sm:w-auto">
                ...
                <table class="w-full text-left border-collapse min-w-[950px]">
                    <thead>
                        <tr class="text-[10px] uppercase font-black text-slate-400 tracking-wider bg-slate-50/50 border-b border-slate-100">
                            ...
                            <th class="px-6 py-4">Serial Numbers</th>
                            <th class="px-6 py-4 text-right">Action</th>
                        </tr>
                    </thead>
                    <tbody id="assetTableBody" class="text-xs font-bold text-slate-700 divide-y divide-slate-100">
                            ...
                            <tr class="asset-row hover:bg-slate-50/80 transition-colors" 
                                ...
                                <td class="px-6 py-4.5">
                                    <span class="font-extrabold text-slate-900 flex items-center gap-2 group cursor-pointer" 
                                        ...
                                        <i class="fa-solid fa-laptop text-slate-400 group-hover:text-indigo-600 transition-colors"></i>
                                        {{ $item->barcode }}
                                    </span>
                                </td>
                                <td class="px-6 py-4.5">
                                    <div class="flex items-center gap-1.5 flex-wrap max-w-[250px]">
                                        <span class="font-extrabold text-slate-900 truncate max-w-[170px] inline-block" title="{{ $item->name ?: 'IT Asset' }}">
                                            {{ $item->primary_name }}
                                        </span>
                                        @if ($item->total_parsed_count > 1)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-black bg-indigo-50 text-indigo-700 border border-indigo-200 shrink-0" title="Total items in this entry: {{ $item->total_parsed_count }}">
                                ...
                                <td class="px-6 py-4.5 text-right">
                                    <a href="{{ route('user.it-assets.show', $item->id) }}"
                                       class="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-bold text-slate-700 shadow-sm transition hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-600">
